...
This manual describes the process of claiming reward, including qubebridge fee, pegbridge fee and staking reward.
...
- [Claim qubebridge fee](#claim-qubebridge-fee)
...
## Claim qubebridge fee

Query all claimable qubebridge fee, as well as token amount and token value in usd:
```shell
sgnd query qubebridge fee-share <validator-eth-address>
```

Along with a `wdlist` flag, we can generate a withdrawal list which will be used later.
```shell
sgnd query qubebridge fee-share <validator-eth-address> --wdlist
...
```shell
sgnd query qubebridge fee-share <validator-eth-address> --wdlist --min-usd 100
```

Once you decided your favorite withdrawal list, create a new file and save it. Then we're ready to claim qubebridge fee.
```shell
touch claim_fee.txt
// use your favorite text editor to write the withdrawal list in and save it.
// submit a sgn transaction
echo $COSMOS_KEYRING_PASSPHRASE | sgnd tx qubebridge validator-claim-fee --file claim_fee.txt
...
If none obvious error happens, congratulations, we've done for claiming qubebridge fee.


## Claim pegbridge fee

The process of claiming pegbridge fee is very similar to claiming qubebridge fee.
